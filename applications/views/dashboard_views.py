        ...
        if date_field and uid:
            try:
                ...
                if hasattr(date_field, 'astimezone'):
                    ...
                    record_date = date_field.date() if hasattr(date_field, 'date') else None
                
                # Only include today's records
                if record_date == today:
                    attendance_list.append({
                        'teacherName': users_dict.get(uid, {}).get('name', 'Unknown'),
                        'subjectName': class_dict.get(classId, {}).get('subjectName', 'Unknown'),
