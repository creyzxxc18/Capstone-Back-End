...
async function fetchAllClasses(department = "all") {
    try {
        ...
        const response = await fetch(`get_classes_list/?department=${department}`);
...
async function showTeacherScheduleModal(teacher) {
    ...
    if (modalHeader) {
        modalHeader.innerHTML = `
            <div style="flex: 1;">
                <h2 id="modalTeacherName" style="margin:0;font-size:20px;font-weight:600;">${teacher.teacher_name || teacher.teacherUid}</h2>
...
async function clearAllClassesLeave(teacherUid) {
    ...
    try {
        const response = await fetch('/clear_all_classes_leave/', {
            method: 'POST',
            headers: {
                'X-CSRFToken': getCookie('csrftoken'),
                'Content-Type': 'application/json',
...
async function markAllClassesLeave(teacherUid, date) {
    ...
    try {
        const response = await fetch('/mark_all_classes_leave/', {
            method: 'POST',
            headers: {
                'X-CSRFToken': getCookie('csrftoken'),
                'Content-Type': 'application/json',
...
async function clearTeacherLeave(attendanceId) {
    ...
    try {
        const response = await fetch('/clear_teacher_leave/', {
            method: 'POST',
            headers: {
                'X-CSRFToken': getCookie('csrftoken'),
                'Content-Type': 'application/json',
...
async function fetchAttendanceForClass(classId, teacherUid, date) {
    try {
        ...
        const url = `/get_attendance_by_class/?classId=${classId}&teacherUid=${teacherUid}&date=${date}`;
        console.log('   URL:', url);
...
async function loadMonthStatuses() {
    try {
        const response = await fetch(`get_month_statuses/?year=${currentYear}&month=${currentMonth}`);
        const data = await response.json();
...
async function setDayStatus(status) {
    ...
    try {
        const response = await fetch('set_day_status/', {
            method: 'POST',
            headers: {
                'X-CSRFToken': getCookie('csrftoken'),
                'Content-Type': 'application/json',
...
async function clearDayStatus() {
    ...
    try {
        const response = await fetch('remove_day_status/', {
            method: 'POST',
            headers: {
                'X-CSRFToken': getCookie('csrftoken'),
                'Content-Type': 'application/json',
