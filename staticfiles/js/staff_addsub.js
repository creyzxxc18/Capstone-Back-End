...
async function fetchClassesByTeacher(teacherUid, department = "all") {
    try {
        ...
        const response = await fetch(`staff_addsub/schedules/get_classes_list/?teacherUid=${teacherUid}&department=${department}`);
...
function deleteSelected() {
    ...
    if (!confirm(`Are you sure you want to delete ${checkboxes.length} subject(s)?`)) {
        return;
    }
    ...
    checkboxes.forEach(checkbox => {
        ...
        if (courseID) {
            const promise = fetch(`staff_addsub/staff_addsub/delete_course/${courseID}/`, {
                method: 'POST',
                headers: {
...
function deleteCourse(classId, btn) {
    ...
    fetch(`staff_addsub/delete_course/${classId}/`, {
        method: "POST",
        headers: {
...
function importExcel() {
    ...
    input.onchange = e => {
        ...
        fetch("staff_addsub/import_class_excel/", {
            method: "POST",
...
function importAllTeachersExcel() {
    ...
    input.onchange = async (e) => {
        ...
        try {
            ...
            const response = await fetch("staff_addsub/import_all_teachers_excel/", {
                method: "POST",
                headers: { "X-CSRFToken": csrftoken },
                body: formData,
            });
...
function editClass(classData) {
    ...
    document.getElementById("editStartTime").value = classData.startTime || "";
    document.getElementById("editEndTime").value = classData.endTime || "";
...
function submitEditClass(event) {
    ...
    fetch(`staff_addsub/update_class/${classId}/`, {
        method: "POST",
        headers: {
            "X-CSRFToken": csrftoken,
            "Content-Type": "application/json",
